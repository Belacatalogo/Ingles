...
function cardFrom(item, index) {
  ...
  return {
    ...
    translation: item?.translation || item?.pt || item?.portuguese || item?.meaning || '',
    definition: item?.definition || item?.meaning || item?.translation || 'Vocabulário da aula atual.',
...
export function FlashcardsScreen({ onNavigate }) {
  ...
  const reviewCards = useMemo(() => getVocabularySrsReviewCards(12).map(cardFrom), [srsVersion]);
  ...
  const activities = useMemo(() => buildVocabularyPracticeActivities(practiceCards, { level: reviewMode ? 2 : level }), [practiceCards, level, reviewMode]);
