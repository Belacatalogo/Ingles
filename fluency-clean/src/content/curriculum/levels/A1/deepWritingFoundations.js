...
export const A1_DEEP_WRITING_FOUNDATIONS = Object.freeze([
  makeWritingLesson({
    ...
    tags: ['a1', 'writing', 'self-introduction', 'controlled-production', 'personal-profile', 'beginner-output'],
    ...
    checklist: [task('Tenho greeting?'), task('Usei My name is?'), task('Usei I am para idade (não I have)?'), task('Usei from para país?'), task('Cada frase carrega uma informação diferente?'), task('Meu texto tem pelo menos 5 frases completas?'), task('Coloquei ponto final em cada frase?'), task('Revisei letras maiúsculas (I e nomes próprios)?')],
    revisionChecklist: [task('Cada frase tem sujeito (I, My name...)?'), task('I está maiúsculo sempre?'), task('Idade usa I am ... years old?'), task('País usa from (não I am Brazil)?'), task('Student tem a antes (I am a student)?'), task('Meu texto responde: nome, idade, país, cidade e papel?')],
  ...
  makeWritingLesson({
    ...
    tags: ['a1', 'writing', 'profile', 'personal-information', 'sentence-writing', 'beginner-output'],
  ...
  makeWritingLesson({
    ...
    tags: ['a1', 'writing', 'family', 'description', 'he-she-pronouns', 'beginner-output'],
  ...
  makeWritingLesson({
    ...
    tags: ['a1', 'writing', 'personal-information', 'countries-cities', 'contact-details', 'beginner-output'],
