...
export const A1_DEEP_LISTENING_FOUNDATIONS = Object.freeze([
  makeListeningLesson({
    ...
    tags: ['a1', 'listening', 'greetings', 'names', 'first-listen', 'beginner-comprehension'],
    ...
    listeningPreparation: [task('Antes de ouvir, pense: quais palavras você espera ouvir num cumprimento?', 'Provavelmente: Hi, Hello, my name is, How are you, good, thanks, Bye.'), task('Feche o transcript na primeira escuta.'), task('Primeira escuta: qual é a situação? (cumprimento, despedida, lição de matemática?)'), task('Segunda escuta: quantas pessoas falam? Você consegue identificar um nome?'), task('Nota: este áudio é gerado por voz sintetizada (TTS). Na vida real, o inglês falado tem mais variação e ritmo. Shadowing ajuda a preparar o ouvido para esse ritmo.')],
...
Ana: Bye, Luis.`,
    ...
    listeningComprehension: [q('Who says “my name is Ana”?', 'Ana', 'O primeiro falante se apresenta.', ['Ana','Luis','Teacher']), q('Who says “my name is Luis”?', 'Luis', 'O segundo falante se apresenta.', ['Luis','Ana','Maria']), q('Áudio da aula — what question does Luis ask?', 'How are you?', 'Ele pergunta estado.', ['How are you?','Where are you from?','What is your phone?']), q('How is Ana?', 'She is good.', 'Ana diz I am good, thanks.', ['She is good.','She is late.','She is from Brazil.']), q('Áudio da aula — how is Luis?', 'He is fine.', 'Luis diz I am fine.', ['He is fine.','He is Ana.','He is online.']), q('Áudio da aula — what word ends the dialogue?', 'Bye', 'Os dois dizem Bye.', ['Bye','Hello','Thanks']), q('Áudio da aula — how many people speak?', 'Two', 'Os personagens do áudio são Ana e Luis.', ['Two','Three','One']), q('Áudio da aula — what is the main situation?', 'They meet and greet each other.', 'É uma apresentação simples.', ['They meet and greet each other.','They buy coffee.','They read a profile.'])],
  ...
  makeListeningLesson({
    ...
    tags: ['a1', 'listening', 'spelling', 'alphabet', 'dictation', 'beginner-comprehension'],
    ...
    listeningPreparation: [task('Antes de ouvir, pense: em que situação alguém pediria para soletrar um nome?', 'Cadastro, chamada, primeira aula — situações em que o nome não é conhecido.'), task('Feche o transcript na primeira escuta.'), task('Prepare a pergunta: Can you spell that?'), task('Prepare letras: A, E, I, L, S, R, Y — essas causam mais confusão para brasileiros.'), task('Primeira escuta: é uma aula? um cadastro? Quem está pedindo o nome?'), task('Nota: em inglês falado real, letras como "A" e "R" soam diferente do português. Ouça com atenção antes de comparar com o transcript.')],
...
Student: Luis.`,
    ...
    listeningComprehension: [q('What is the student’s first name?', 'Luis', 'Ele diz My name is Luis Silva e soletra L-U-I-S.', ['Luis','Silva','Ana']), q('What is the student’s last name?', 'Silva', 'Ele soletra S-I-L-V-A.', ['Silva','Luis','Santos']), q('What does “spell” mean here?', 'soletrar', 'A professora pede letras.', ['soletrar','comprar','escutar música']), q('Áudio da aula — what does the teacher ask first?', 'What is your name?', 'Primeira fala da professora.', ['What is your name?','How old are you?','Where are you from?']), q('Does the student spell his last name?', 'Yes, he does', 'Ele diz S-I-L-V-A.', ['Yes, he does','No, he does not','Only his first name']), q('Áudio da aula — which word makes the request polite?', 'please', 'A professora diz please.', ['please','great','name']), q('Áudio da aula — what does repeat mean?', 'repetir', 'Please repeat.', ['repetir','soletrar','sair']), q('How many names are confirmed?', 'Two', 'First name and last name.', ['Two','One','Three'])],
  ...
  makeListeningLesson({
    ...
    tags: ['a1', 'listening', 'numbers', 'phone-numbers', 'dictation', 'beginner-comprehension'],
...
Student: Five five zero three two nine.`,
    ...
    listeningComprehension: [q('Áudio da aula — what is her name?', 'Ana', 'Ela diz My name is Ana.', ['Ana','Luis','Carla']), q('How old is Ana?', '19', 'Ela diz nineteen years old.', ['19','20','9']), q('Áudio da aula — what information does the clerk ask after age?', 'phone number', 'What is your phone number?', ['phone number','city','country']), q('What is the first digit of the phone number?', 'five', 'five five zero...', ['five','zero','three']), q('Which digit comes after two fives?', 'zero', 'five five zero...', ['zero','two','nine']), q('Does Ana repeat the number?', 'Yes, she does', 'Ela repete o número.', ['Yes, she does','No, she does not','Only her name']), q('What does repeat mean?', 'repetir', 'Can you repeat that?', ['repetir','soletrar','sair']), q('Áudio da aula — what is the situation?', 'A simple registration.', 'Clerk asks personal information.', ['A simple registration.','A family dinner.','A reading class.'])],
  ...
  makeListeningLesson({
    ...
    tags: ['a1', 'listening', 'countries', 'cities', 'personal-information', 'beginner-comprehension'],
...
Ana: Bruno is from Brazil too. He lives in Curitiba.`,
    ...
    listeningComprehension: [q('Áudio da aula — where is Ana from?', 'Brazil', 'Ela diz I am from Brazil.', ['Brazil','Curitiba','Recife']), q('Is Ana Brazilian?', 'Yes, she is', 'Ela diz Yes, I am Brazilian.', ['Yes, she is','No, she is not','Not mentioned']), q('What is Ana’s city?', 'Recife', 'Ela diz My city is Recife.', ['Recife','Curitiba','Brazil']), q('Where does Ana live?', 'Recife', 'Ela diz I live in Recife.', ['Recife','Brazil','Canada']), q('Where does Bruno live?', 'Curitiba', 'He lives in Curitiba.', ['Curitiba','Recife','Brazilian']), q('Is Bruno from Brazil too?', 'Yes', 'Bruno is from Brazil too.', ['Yes','No','Only Ana']), q('Áudio da aula — what word signals origin?', 'from', 'I am from Brazil.', ['from','city','teacher']), q('Áudio da aula — what phrase means morar em?', 'live in', 'I live in Recife.', ['live in','from','years old'])],
