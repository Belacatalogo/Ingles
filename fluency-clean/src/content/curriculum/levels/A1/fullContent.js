...
function grammarLesson([n, title, pattern, chunks, examples]) {
  ...
  return createGrammarLesson({
    ...
    transformationPractice: [
      ...
      q(`${title} — complete com uma palavra estrutural correta.`, ['is', 'blue', 'table'], 'is', 'A palavra estrutural necessária é o verbo to be (is) que liga sujeito ao complemento.'),
...
function vocabularyLesson([n, title, words]) {
  return createVocabularyLesson({
    ...
    recognitionPractice: [
      q(`Qual palavra pertence ao tema ${title}?`, [words[0], 'airplane engine', 'complex grammar'], words[0], `A palavra ${words[0]} faz parte do vocabulário de ${title} estudado nesta aula.`),
      q(`${title} — qual opção é mais útil para A1?`, [words[1], 'philosophy', 'industrialization'], words[1], `${words[1]} é uma palavra útil para A1. As outras são muito avançadas para este nível.`),
      ...
      q(`${title} — qual palavra você pode usar em frase curta?`, [words[3], 'constitutional', 'incomprehensible'], words[3], `${words[3]} pode ser usada em frases A1 simples. As outras são formais demais.`),
