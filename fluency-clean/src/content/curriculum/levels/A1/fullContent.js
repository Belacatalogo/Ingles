...
function grammarLesson([n, title, pattern, chunks, examples]) {
  ...
  return createGrammarLesson({
    ...
    guidedPractice: [
      ...
      q(`Complete o padrão: ${pattern}`, chunks.slice(0, 3), chunks[0], `O chunk correto completa o padrão ${pattern} da aula.`),
      q(`${title} — escolha a frase mais natural.`, [examples[1] || examples[0], 'She are a student.', 'I has a book.'], examples[1] || examples[0], `A frase segue o padrão ${pattern} de forma natural e correta.`),
      q(`${title} — qual opção mantém a ordem do inglês?`, [examples[2] || examples[0], 'Very happy she is.', 'Book my is.'], examples[2] || examples[0], 'Em inglês A1, a ordem é sujeito + verbo + complemento. As outras invertem essa ordem.'),
      q(`${title} — qual opção é A1 correta?`, [examples[3] || examples[0], 'I can to speak.', 'Does she studies?'], examples[3] || examples[0], 'A opção correta não usa to depois de can nem duplica a marca de terceira pessoa.'),
      q(`${title} — marque a alternativa segura para iniciante.`, [examples[0], 'Advanced mixed sentence', 'Portuguese word order'], examples[0], `A frase-modelo da aula de ${title} é segura porque segue o padrão sem traduzir do português.`),
      q(`${title} — qual frase você pode repetir como modelo?`, [examples[1] || examples[0], 'Bad model', 'Incomplete'], examples[1] || examples[0], 'O modelo tem sujeito, verbo e complemento. As outras opções estão incompletas.'),
      q(`${title} — qual opção evita tradução palavra por palavra?`, [examples[0], 'I with hungry am.', 'The my book blue is.'], examples[0], 'As opções erradas copiam a ordem do português. A correta segue a estrutura do inglês.'),
      q(`${title} — escolha a frase com estrutura completa.`, [examples[2] || examples[0], 'My brother happy.', 'On table book.'], examples[2] || examples[0], 'A frase completa tem sujeito + verbo + complemento. As outras pulam o verbo.'),
      q(`${title} — qual opção deve virar modelo de produção?`, [examples[3] || examples[0], 'Wrong order', 'Missing verb'], examples[3] || examples[0], `A opção correta pode ser usada como modelo para criar frases novas com ${title}.`),
    ],
    transformationPractice: [
      q(`${title} — transforme em frase negativa quando possível.`, ['I do not study every day.', 'I not study every day.', 'I study not every day.'], 'I do not study every day.', 'No present simple negativo, use do not antes do verbo base. Não coloque not sozinho.'),
      q(`${title} — transforme em pergunta quando possível.`, ['Do you study English?', 'You do study English?', 'Study you English?'], 'Do you study English?', 'Na pergunta do present simple, Do vem antes do sujeito: Do you + verbo base.'),
      q(`${title} — corrija a ordem da frase.`, ['I study English at night.', 'I English study at night.', 'At night English I study.'], 'I study English at night.', 'A ordem correta em inglês é sujeito + verbo + objeto + tempo.'),
      q(`${title} — escolha a versão mais simples e correta.`, ['She is my friend.', 'She my friend is.', 'She friend my.'], 'She is my friend.', 'She is my friend segue sujeito + verbo to be + complemento.'),
      q(`${title} — complete com uma palavra estrutural correta.`, ['is', 'do', 'the'], 'is', 'A palavra estrutural necessária é o verbo to be (is) que liga sujeito ao complemento. "Do" é auxiliar de ação e "the" é artigo definido — nenhum dos dois completa a ligação sujeito–complemento aqui.'),
      q(`${title} — escolha a frase pronta para falar.`, ['Can you repeat, please?', 'You can to repeat?', 'Repeat can you please?'], 'Can you repeat, please?', 'Depois de can, use verbo base sem to. A ordem é Can + sujeito + verbo.'),
    ],
...
function vocabularyLesson([n, title, words]) {
  return createVocabularyLesson({
    ...
    teacherOpening: `Nesta aula vamos trabalhar vocabulário de ${title}. O objetivo é reconhecer, usar e produzir palavras úteis do tema.`,
    lessonRecap: [`Você conclui ${title} quando consegue reconhecer e usar as palavras em frases curtas.`],
    recognitionPractice: [
      q(`Qual palavra pertence ao tema ${title}?`, [words[0], 'paragraph', 'algorithm'], words[0], `A palavra ${words[0]} faz parte do vocabulário de ${title} estudado nesta aula.`),
      q(`${title} — qual opção é mais útil para A1?`, [words[1], 'therefore', 'meanwhile'], words[1], `${words[1]} é uma palavra útil para A1. As outras são muito avançadas para este nível.`),
      q(`${title} — escolha uma palavra da aula.`, [words[2], 'random', 'unknown'], words[2], `${words[2]} aparece no vocabulário de ${title}. As outras não pertencem ao tema.`),
      q(`${title} — qual palavra você pode usar em frase curta?`, [words[3], 'contract', 'schedule'], words[3], `${words[3]} pode ser usada em frases A1 simples. As outras são formais demais.`),
      q(`${title} — qual opção combina com o vocabulário estudado?`, [words[4], 'unrelated', 'wrong'], words[4], `${words[4]} pertence ao tema ${title} da aula e pode ser usada em contexto real.`),
      q(`${title} — escolha uma palavra para revisar.`, [words[5], 'no option', 'bad option'], words[5], `Revisar ${words[5]} reforça o vocabulário de ${title} ensinado nesta aula.`),
    ],
...
const READING_TOPICS = [
  ...
  [18, 'Vocabulary from context', 'I am hungry, so I eat a sandwich. I am thirsty, so I drink water. I am tired, so I rest. I am cold, so I wear a jacket. I am happy, so I smile. When I do not know a word, I read the whole sentence first. The other words help me. For example, in "I am sick, so I see a doctor," the word doctor gives me a clue about the word sick. These words are easy when the sentence gives you a clue.'],
...
function readingLesson([n, title, text]) {
  return createReadingLesson({
    ...
    comprehensionQuestions: [q(`${title} — qual é a ideia geral do texto?`, ['informação simples A1', 'história policial', 'texto técnico'], 'informação simples A1', 'O texto apresenta informações pessoais e rotina, típicas do nível A1.'), q(`${title} — o texto tem nomes ou pessoas?`, ['Sim', 'Não'], 'Sim', 'Procure nomes próprios e pronomes como I, she, he no texto.'), q(`${title} — o texto tem ações ou descrição?`, ['Sim', 'Não'], 'Sim', 'O texto descreve ações diárias e características de pessoas ou lugares.'), q(`${title} — há informação que pode ser provada no texto?`, ['Sim', 'Não'], 'Sim', 'Informações como nomes, lugares e números podem ser confirmadas relendo o texto.'), q(`${title} — o texto é adequado para iniciante?`, ['Sim', 'Não'], 'Sim', 'O texto usa frases curtas, vocabulário simples e estruturas A1.'), q(`${title} — você deve responder com evidência?`, ['Sim', 'Não'], 'Sim', 'Sempre copie uma frase do texto que comprove sua resposta.')],
...
function listeningLesson([n, title, script]) {
  return createListeningLesson({
    ...
    lessonRecap: [`Você conclui ${title} quando consegue identificar as palavras-chave e repetir frases do áudio.`],
    listeningComprehension: [q(`${title} — o áudio tem informação simples?`, ['Sim', 'Não'], 'Sim', 'O áudio usa frases curtas e vocabulário básico de A1.'), q(`${title} — você deve ouvir antes de ler?`, ['Sim', 'Não'], 'Sim', 'Primeiro ouça sem ler para treinar compreensão auditiva.'), q(`${title} — há palavras A1 no áudio?`, ['Sim', 'Não'], 'Sim', 'O áudio contém palavras-chave como name, family, study, work.'), q(`${title} — o áudio deve ser repetido em voz alta?`, ['Sim', 'Não'], 'Sim', 'Repetir em voz alta (shadowing) ajuda a fixar pronúncia e ritmo.')],
