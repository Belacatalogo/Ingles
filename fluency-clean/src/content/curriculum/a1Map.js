...
function simple(titles, packageRules, objectivePrefix, tag) {
  return titles.map((title, index) => item(title, packageRules(index + 1), `${objectivePrefix}: ${title}.`, [tag, title.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '')]));
}
