...
const TYPE_REQUIREMENTS = {
  ...
  listening: {
    ...
    minPrompts: 4,
    minMainLength: 750,
    requiredSignals: ['escuta', 'detalhe', 'shadowing', 'vocabulário', 'revisão'],
  },
  writing: {
    minSections: 6,
    minVocabulary: 10,
...
function requirementFor(type) {
  return TYPE_REQUIREMENTS[type] || TYPE_REQUIREMENTS.reading;
}
...
function scoreCount(count, required) {
  if (!required) return 100;
  return normalizeScore((count / required) * 100);
}

function scoreSignals(searchText, signals) {
  if (!signals.length) return 100;
  const matched = signals.filter((signal) => searchText.includes(signal.toLowerCase())).length;
  return normalizeScore((matched / signals.length) * 100);
}
...
function buildReview({ lesson, expectedLevel }) {
  ...
  const sectionDepthScore = lesson.sections.length
    ? normalizeScore((lesson.sections.filter((section) => countWords(section.content) >= 18).length / lesson.sections.length) * 100)
    : 0;
  ...
  const vocabularyQuality = lesson.vocabulary.length
    ? normalizeScore((lesson.vocabulary.filter((item) => cleanText(item.word) && cleanText(item.meaning) && cleanText(item.example)).length / lesson.vocabulary.length) * 100)
    : 0;
  ...
  const completeness = normalizeScore(
    (sectionCountScore * 0.2) +
    (mainContentScore * 0.15) +
    (vocabularyScore * 0.15) +
    (vocabularyQuality * 0.1) +
    (exerciseCountScore * 0.2) +
    (reviewIncluded * 0.2)
  );

  const progression = normalizeScore((sectionDepthScore * 0.35) + (signalScore * 0.35) + (reviewIncluded * 0.3));
  ...
  return {
    approved: overallScore >= APPROVAL_SCORE && uniqueIssues.length <= 3,
    overallScore,
    objectiveClear,
    levelFit,
    clarity,
    ...
    reviewIncluded,
    issues: uniqueIssues,
    revisionInstructions: uniqueIssues.length
      ? `Corrija antes de salvar: ${uniqueIssues.join(' ')}`
      : '',
...
export function validateLessonForQuality(rawLesson, { expectedLevel = '', expectedType = '' } = {}) {
  ...
  return buildReview({ lesson, expectedLevel: expectedLevel || lesson.level });
}
