...
export function isValidGeminiKey(key) { return /^AIza[0-9A-Za-z_\-]{20,}$/.test(String(key ?? '').replace(/\s+/g, '').trim()); }
export function normalizeLessonKeys(keys) {
  const source = Array.isArray(keys) ? keys : String(keys ?? '').split(/[\n,;| ]+/);
  const seen = new Set();
  return source.map((key) => String(key ?? '').replace(/\s+/g, '').trim()).filter((key) => {
...
function getHttpStatus(error) { const match = String(error?.message || error).match(/HTTP\s+(\d{3})/i); return match ? Number(match[1]) : null; }
...
function validateGeneratedLesson(lesson) {
  ...
  const hasStudyContent = normalized.intro || normalized.sections.length > 0 || normalized.vocabulary.length > 0 || normalized.exercises.length > 0 || normalized.listeningText || normalized.prompts.length > 0;
...
export async function generateLessonDraft({ prompt, keys = [], proKey = '', fetcher = fetch, previousLesson = null, forceVariation = false } = {}) {
  ...
  if (!attempts.length) {
    ...
    return { status: GEMINI_LESSON_STATUS.missingKeys, lesson: null, error: 'Nenhuma key Gemini válida configurada para aulas.' };
  }
  ...
  return { status: GEMINI_LESSON_STATUS.error, lesson: null, error: finalError };
}
