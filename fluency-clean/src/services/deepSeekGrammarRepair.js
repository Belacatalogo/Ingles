...
function summarizeLesson(lesson) {
  return {
    ...
    exercises: ensureArray(lesson?.exercises).map((item) => ({ question: clean(item?.question), options: ensureArray(item?.options).slice(0, 4), answer: clean(item?.answer), explanation: clean(item?.explanation) })).slice(0, 18),
...
function buildPrompt(lesson) {
  return [
    ...
    'Crie 16 a 20 exercícios em português para A1 com options, answer e explanationPt. answer deve ser exatamente uma das options.',
    'Misture escolha correta, completar lacuna, corrigir erro, transformar frase e tradução curta controlada.',
    'Crie 5 a 7 prompts de produção própria em português com inglês simples.',
...
function normalizeExercises(exercises) {
  ...
  })).filter((item) => item.question && item.options.length >= 2 && item.answer && item.options.some((option) => option.toLowerCase() === item.answer.toLowerCase())).slice(0, 20);
}
...
function validateRepair(data) {
  ...
  return { sections, exercises, prompts, approved: sections.length >= 7 && exercises.length >= 12 && prompts.length >= 4 && hits >= 5 };
}
async function callDeepSeek({ prompt, key, model, fetcher }) {
  const response = await fetcher('https://api.deepseek.com/chat/completions', {
    method: 'POST', headers: { 'content-type': 'application/json', Authorization: `Bearer ${key}` },
    body: JSON.stringify({ model, messages: [{ role: 'system', content: 'Você responde apenas JSON válido. A palavra json é obrigatória neste modo. Não use markdown.' }, { role: 'user', content: prompt }], temperature: 0.14, max_tokens: 7200, response_format: { type: 'json_object' } }),
  });
...
export async function repairGrammarWithDeepSeek(lesson, { fetcher = fetch } = {}) {
  ...
  if (!validated.approved) {
    diagnostics.log(`DeepSeek Grammar Repair não aprovou estrutura: ${validated.sections.length} seções, ${validated.exercises.length} exercícios, ${validated.prompts.length} prompts.`, 'warn', data);
  ...
  const repairedLesson = {
    ...
    grammarRepair: { provider: 'deepseek', model, appliedAt: new Date().toISOString(), sections: validated.sections.length, exercises: validated.exercises.length, prompts: validated.prompts.length, contract: 'deepseek-grammar-repair-v1' },
    planContract: `${lesson?.planContract || 'lesson-contract-v1'}+deepseek-grammar-repair-v1`,
  };
